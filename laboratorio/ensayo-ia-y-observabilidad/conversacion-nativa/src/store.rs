...
pub struct Database{pub cases:BTreeMap<String,Case>,pub chats:BTreeMap<String,Chat>,pub events:BTreeMap<String,Vec<Event>>,pub bytes:u64,root:PathBuf}
...
impl Database{
 pub fn open(root:PathBuf)->Result<Self>{
  ...
  let mut db=Self{cases:BTreeMap::new(),chats:BTreeMap::new(),events:BTreeMap::new(),bytes:0,root:root.clone()};
...
pub fn check_cli()->Result<()>{let p=std::env::args().nth(2).ok_or("Uso: --check DIRECTORIO")?;let db=Database::open(PathBuf::from(p))?;println!("{}",json!({"integridad":"conforme","expedientes":db.cases.len(),"conversaciones":db.chats.len(),"sucesos":db.events.values().map(Vec::len).sum::<usize>(),"bytes":db.bytes}));Ok(())}
