...
function renderChat(data){current=data;const key=JSON.stringify(data);if(key===lastRender)return;lastRender=key;const box=$('messages'),atEnd=box.scrollTop+box.clientHeight>=box.scrollHeight-70;const oldScroll=box.scrollTop;const opened=[...box.querySelectorAll('details')].map(d=>d.open);
 ...
 if(thinking)detail(a,'Texto de razonamiento generado por Qwen · sin validar',thinking);
 const r=t.result||{};const status=active?'En curso · '+active.seconds+' s':(t.status==='fin_normal'?'Generación terminada · contenido sin validar':t.status.replaceAll('_',' '));a.append(el('div',status+(r.tokens!==undefined?' · '+r.tokens+' unidades generadas':'')+(r.seconds?' · '+r.seconds.toFixed(2)+' s':'')+(r.first_output_seconds?' · primera salida del proceso '+r.first_output_seconds.toFixed(2)+' s':''),'meta'));
