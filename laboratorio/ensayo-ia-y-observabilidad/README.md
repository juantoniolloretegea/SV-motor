...
**Base técnica revisada:** [código y expedientes en `d82bbe2`](https://github.com/juantoniolloretegea/SV-motor/tree/d82bbe2f5f396eb31da5b095ba0849404cb352b0/laboratorio/ensayo-ia-y-observabilidad). La revisión documental 2.3 no incorpora nuevas ejecuciones.

**Lectura:** [objeto](#1-objeto-adscripción-y-autoridad) · [resultados y pendientes](#2-estado-de-la-evidencia-en-el-corte-revisado) · [criterio de avance](#3-regla-de-avance-entre-las-dos-vías) · [componentes y términos](#4-componentes-comunes-y-función-de-cada-uno) · [dos vías y diagramas](#5-dos-vías-de-ejecución-para-un-objetivo-común) · [seguridad](#6-seguridad-integridad-y-observabilidad) · [continuación](#7-secuencia-y-organización-del-trabajo) · [procedencia desde la versión 0.1](#8-procedencia-y-antecedente-documental-conservado).
...
Las dos vías comparten ese objeto: **A** sitúa la inferencia en el navegador mediante WASM; **B** la sitúa en un proceso nativo al que la página remite solicitudes. Se distinguen sus mecanismos y evidencias; el avance se organiza de forma secuencial.
...
## 2. Estado de la evidencia en el corte revisado
...
| Ajustes estáticos de la candidata nativa | NAT02-A: el banco no excluye ciertos rechazos genéricos sin la evidencia exigida por caso y la recuperación carece de una cota global de progreso. NAT02-B: el fallo de custodia al registrar `waitpid` puede propagarse antes de ofrecer un cierre consultable. | Caminos identificados por inspección de fuentes, no fallos reproducidos. Corrección y comprobación pendientes; no habilitan una inferencia. |
...
- [Resultado EIO-05](https://github.com/juantoniolloretegea/SV-motor/blob/d82bbe2f5f396eb31da5b095ba0849404cb352b0/laboratorio/ensayo-ia-y-observabilidad/resultados/continuacion-05/RESULTADO.md) y [resultado EIO-06](https://github.com/juantoniolloretegea/SV-motor/blob/d82bbe2f5f396eb31da5b095ba0849404cb352b0/laboratorio/ensayo-ia-y-observabilidad/resultados/continuacion-06/RESULTADO.md).
- [Banco de API: oráculos y recuperación](https://github.com/juantoniolloretegea/SV-motor/blob/d82bbe2f5f396eb31da5b095ba0849404cb352b0/laboratorio/ensayo-ia-y-observabilidad/resultados/preparacion-nativa-02/pruebas/api_real.rs) y [supervisor: tratamiento de la recogida del proceso hijo](https://github.com/juantoniolloretegea/SV-motor/blob/d82bbe2f5f396eb31da5b095ba0849404cb352b0/laboratorio/ensayo-ia-y-observabilidad/resultados/preparacion-nativa-02/nativa/supervisor.rs), fuentes de los ajustes estáticos NAT02-A y NAT02-B.
...
## 3. Regla de avance entre las dos vías
...
## 4. Componentes comunes y función de cada uno
...
**Términos de lectura.** Un *control* es una comprobación delimitada; un *oráculo* fija lo esperado antes de observar el resultado. *Admisibilidad técnica* significa que se cumplen las condiciones de ejecución y custodia del contrato; no equivale a corrección del consejo ni a autorización humana.

| Término | Significado en este documento |
|---|---|
| API / HTTP | Frontera de solicitudes y respuestas del servicio / protocolo empleado por la página para comunicarse con él. |
| Web Worker / WASM | Contexto de trabajo del navegador / formato de compilación y ejecución del módulo Rust. Son componentes distintos. |
| Custodia / sello / snapshot | Conservación de evidencia / identificador ligado a su manifiesto / copia estable de los bytes recuperables. El sello aquí descrito no es una firma digital ni un sello temporal independiente. |
| RSS / PSS | Memoria residente contabilizada por proceso / medida que reparte proporcionalmente las páginas compartidas. La suma de RSS puede contar páginas compartidas más de una vez; no equivale a memoria física exclusiva. |
| cgroup v2 | Mecanismo de Linux para agrupar procesos y aplicar controles. Su disponibilidad y permisos deben comprobarse en el anfitrión. |
| EOF / lock | Fin observado de un canal de lectura / archivo Cargo.lock que fija la resolución de dependencias. Ninguno acredita por sí solo una ejecución correcta. |

## 5. Dos vías de ejecución para un objetivo común
...
### 5.1. Distribución de responsabilidades entre Rust y JavaScript
...
Reducir las responsabilidades de JavaScript no demuestra por sí solo una reducción global de fallos. Eliminarlo también de la interfaz supondría otra realización, no incluida en esta candidata. La prioridad entre las dos vías sigue regida por el apartado 3, sin preferencia automática por lenguaje o número de componentes.

### 5.2. Diagrama de la vía A: navegador y WASM
...
La verificación de recursos se describe con la reserva del apartado 6: no constituye autenticación independiente previa de todo el JavaScript ejecutado. Los canales instrumentados tampoco acreditan la ausencia universal de otras actividades.

Fuentes del diagrama: [página de control](https://github.com/juantoniolloretegea/SV-motor/blob/d82bbe2f5f396eb31da5b095ba0849404cb352b0/laboratorio/ensayo-ia-y-observabilidad/resultados/preparacion-navegador-02/web/control.js), [Worker](https://github.com/juantoniolloretegea/SV-motor/blob/d82bbe2f5f396eb31da5b095ba0849404cb352b0/laboratorio/ensayo-ia-y-observabilidad/resultados/preparacion-navegador-02/web/worker.js), [enlace y verificación Rust](https://github.com/juantoniolloretegea/SV-motor/blob/d82bbe2f5f396eb31da5b095ba0849404cb352b0/laboratorio/ensayo-ia-y-observabilidad/resultados/preparacion-navegador-02/pruebas/navegador.rs), [controlador exterior](https://github.com/juantoniolloretegea/SV-motor/blob/d82bbe2f5f396eb31da5b095ba0849404cb352b0/laboratorio/ensayo-ia-y-observabilidad/resultados/preparacion-navegador-02/controlador.mjs) y [resultado NAV-02](https://github.com/juantoniolloretegea/SV-motor/blob/d82bbe2f5f396eb31da5b095ba0849404cb352b0/laboratorio/ensayo-ia-y-observabilidad/resultados/navegador-02/README.md), en el corte de código indicado al inicio.

### 5.3. Diagrama de la vía B: servicio nativo e interfaz web
...
Fuentes del diagrama: [interfaz JavaScript](https://github.com/juantoniolloretegea/SV-motor/blob/d82bbe2f5f396eb31da5b095ba0849404cb352b0/laboratorio/ensayo-ia-y-observabilidad/resultados/preparacion-nativa-02/web/app.js), [API Rust](https://github.com/juantoniolloretegea/SV-motor/blob/d82bbe2f5f396eb31da5b095ba0849404cb352b0/laboratorio/ensayo-ia-y-observabilidad/resultados/preparacion-nativa-02/nativa/servidor.rs), [supervisor](https://github.com/juantoniolloretegea/SV-motor/blob/d82bbe2f5f396eb31da5b095ba0849404cb352b0/laboratorio/ensayo-ia-y-observabilidad/resultados/preparacion-nativa-02/nativa/supervisor.rs), [proceso de inferencia](https://github.com/juantoniolloretegea/SV-motor/blob/d82bbe2f5f396eb31da5b095ba0849404cb352b0/laboratorio/ensayo-ia-y-observabilidad/resultados/preparacion-nativa-02/nativa/inferidor.rs), [custodia](https://github.com/juantoniolloretegea/SV-motor/blob/d82bbe2f5f396eb31da5b095ba0849404cb352b0/laboratorio/ensayo-ia-y-observabilidad/resultados/preparacion-nativa-02/nativa/custodia.rs), [guarda exterior](https://github.com/juantoniolloretegea/SV-motor/blob/d82bbe2f5f396eb31da5b095ba0849404cb352b0/laboratorio/ensayo-ia-y-observabilidad/resultados/preparacion-nativa-02/nativa/guarda.rs) y [diseño y reservas](https://github.com/juantoniolloretegea/SV-motor/blob/d82bbe2f5f396eb31da5b095ba0849404cb352b0/laboratorio/ensayo-ia-y-observabilidad/resultados/preparacion-nativa-02/DISENO.md), en el corte de código indicado al inicio.

Los diagramas separan petición, cálculo, verificación, observación, control y custodia. Su orden representa relaciones funcionales; no constituye una medida temporal ni una prueba de independencia entre componentes. La regla de prioridad del apartado 3 se aplica al cumplimiento del conjunto.

## 6. Seguridad, integridad y observabilidad
...
## 8. Procedencia y antecedente documental conservado
...
La [adición de enlaces de procedencia a la versión 2.2.1](https://github.com/juantoniolloretegea/SV-motor/blob/3d185b27c6bb728f1b23509d53dc6ad99c52883d/laboratorio/ensayo-ia-y-observabilidad/README.md) conserva su identidad propia.

**Revisión 2.3:** orden de lectura orientado a objeto, evidencia, criterio de avance y arquitectura; fuentes de las figuras fijadas al corte técnico; términos definidos y ajustes estáticos pendientes explicitados. No modifica código candidato, resultados, controles ni autorizaciones.

**Función del desplegable.** Es una copia histórica de la versión 0.2 con sus notas posteriores, conservada al elaborar la versión 2.0; no equivale exclusivamente a la primera publicación de 0.2. Permite reconstruir el desarrollo del ensayo. Las vías nativa y WASM ya estaban previstas en los antecedentes; la revisión actual precisa su arquitectura y el servicio nativo posterior. Los estados históricos no se trasladan al presente ni constituyen una habilitación vigente.

La versión 0.2 y sus notas de continuidad se mantienen a continuación sin alterar su texto. Sus estados y expresiones temporales corresponden a los momentos documentados; la síntesis vigente de este README es la versión 2.3 de los apartados 1–7. Las revisiones del contrato y de los expedientes mantienen sus identidades propias.
