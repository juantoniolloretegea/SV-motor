...
impl Config{
 pub fn comprobar(&self)->Result<(),Error>{
  ...
  if !["normal","bloqueo","abrupto","omision","duplicado","orden","hostil","memoria","cola","registro","escritura","no_cero","eof_ausente","tardio","bloqueo_antes","bloqueo_entre","bloqueo_sellado","fallo_sync","cola_pendiente"].contains(&self.caso.as_str()){return Err("CASO".into())}
  Ok(())
 }
}
#[derive(Debug,Serialize,Deserialize)]
