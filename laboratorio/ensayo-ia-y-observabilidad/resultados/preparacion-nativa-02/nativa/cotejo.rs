...
#[derive(Deserialize)]#[serde(deny_unknown_fields)]
struct Manifiesto{contrato:String,estado:String,completa:bool,corte_custodia:u64,laguna_tardia_posible:bool,archivos:Vec<Item>,coste_escritura_ns:u128,escrituras:u64,excluido:String}
fn main()->Result<(),Error>{
 ...
 let m:Manifiesto=serde_json::from_slice(&b)?;if m.contrato!="EIO-NAT/2"||m.archivos.len()!=4{return Err("INVENTARIO".into())}
 ...
 println!("{}",serde_json::json!({"estado_emisor":m.estado,"corte_custodia":m.corte_custodia,"laguna_tardia_posible":m.laguna_tardia_posible,"completitud_declarada_emisor":m.completa,"coste_escritura_ns":m.coste_escritura_ns,"escrituras":m.escrituras,"excluido":m.excluido,"alcance":"identidad de cuatro archivos; no prueba exhaustividad ni aceptación"}));Ok(())
}
