...
fn bytes(seq:u64,tipo:&str,datos:serde_json::Value)->Vec<u8>{
 serde_json::to_vec(&Frame{contrato:"EIO-NAT/1".into(),id:"PRUEBA-01".into(),seq,mono_ns:0,civil_unix_ms:0,tipo:tipo.into(),datos}).unwrap()
}
...
#[test]fn entrada_literal(){
 for p in ["referencia","estructurada"]{let t=literal(p).unwrap();assert_eq!(validar_inicio("EIO-NAT/1",p,t),Ok(()));assert_eq!(validar_inicio("EIO-NAT/1",p,&format!("{t} ")),Err("PETICION_NO_FIJADA"));}
 assert_eq!(validar_inicio("EIO-NAT/2","referencia",""),Err("VERSION"));
 assert_eq!(validar_inicio("EIO-NAT/1","libre","hola"),Err("PETICION_NO_FIJADA"));
}
