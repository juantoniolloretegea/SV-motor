//! EIO-NAT/1. Candidata Linux no compilada ni ejecutada.
...
impl Config{
 pub fn comprobar(&self)->Result<(),Error>{
  if !self.habilitada || self.contrato!="EIO-NAT/1" {return Err("CONFIGURACION_INACTIVA".into())}
  ...
  if !["normal","bloqueo","abrupto","omision","duplicado","orden","hostil","memoria","cola","registro","escritura"].contains(&self.caso.as_str()){return Err("CASO".into())}
  Ok(())
 }
}
#[derive(Debug,Serialize,Deserialize)]
...
pub enum Orden {
 ...
 #[serde(rename="evidencia")] Evidencia{archivo:String,offset:u64},
}
pub fn validar_inicio(contrato:&str,p:&str,bytes:&str)->Result<(),&'static str>{
 if contrato!="EIO-NAT/1"{return Err("VERSION")}
...
pub fn emitir(tipo:&str,datos:Value)->Result<(),Error>{
 ...
 let f=Frame{contrato:"EIO-NAT/1".into(),id:s.0.clone(),seq:s.1,mono_ns:s.2.elapsed().as_nanos(),civil_unix_ms:civil(),tipo:tipo.into(),datos};
...
impl Oracle{
 pub fn recibir(&mut self,b:&[u8],id:&str)->Result<(),Error>{
  ...
  if f.contrato!="EIO-NAT/1"||f.id!=id||f.seq!=self.seq+1||self.resultado.is_some(){self.error=true;return Err("IDENTIDAD_SECUENCIA_O_TARDIO".into())}
  self.seq=f.seq;
  match f.tipo.as_str(){
