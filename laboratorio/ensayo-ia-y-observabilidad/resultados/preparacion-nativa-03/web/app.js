...
const $=id=>document.getElementById(id);let identidad=null;let selloSesion=null;
...
async function accion(x){try{
 ...
 $("original").textContent=v.admisible===true?(v.resultado?.salida_original?.texto??"Resultado ausente"):"Sin resultado admisible";
 $("juicio").textContent=v.admisible===true?(v.resultado?.juicio_verificador??"Juicio ausente"):"No admisible técnicamente";
 $("custodia").textContent=v.sello?((v.evidencia_parcial?"EVIDENCIA PARCIAL, no resultado admitido. ":"Conjunto sellado. ")+"Sello: "+v.sello):"Sin conjunto sellado recuperable";
}catch(e){$("salida").textContent="Consulta no confirmada: "+String(e);$("original").textContent="";$("juicio").textContent="Desconocido";}}
for(const p of ["referencia","estructurada"])$(p).onclick=()=>accion({op:"iniciar",contrato:"EIO-NAT/2",peticion:p,texto:peticiones[p]});
...
for(const archivo of ["inferidor.jsonl","inferidor.stderr","supervision.jsonl","entrada.txt","MANIFIESTO.json"]){
 ...
 b.onclick=async()=>{b.disabled=true;try{
  const estado=await api({op:"estado"});
  if(!estado.sello)throw Error("Sin sello");if(selloSesion!==null&&selloSesion!==estado.sello)throw Error("Cambio de conjunto; detener recuperación");selloSesion=estado.sello;
  let offset=0;const partes=[];let total=null;
  for(let i=0;i<257;i++){
   const v=await api({op:"evidencia",archivo,offset,sello:selloSesion});
   if(v.sello!==selloSesion||v.archivo!==archivo||v.offset!==offset||v.total>8388608||(total!==null&&total!==v.total)||!/^(?:[0-9a-f]{2})*$/.test(v.hex))throw Error("Identidad de fragmento");
