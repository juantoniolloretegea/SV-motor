...
pub struct Custodia{
 ...
 pub error:Option<String>,pub fase:&'static str,pub corte:u64,pub tardios_bytes:u64,pub snapshot:Option<Snapshot>,
}
impl Custodia{
 pub fn nueva(dir:PathBuf,caso:String)->Result<Self,Error>{
    ...
    while let Ok(t)=work.recv(){
     match t{
      Trabajo::Datos(n,i,b)=>{if n!=ultimo+1{return Err("COLA_SECUENCIA".into())}ultimo=n;
       ...
       d.escribir(i,&b)?;
       d.evento("trabajo",json!({"ordinal":n,"clase":"datos","archivo":FILES[i],"bytes":b.len(),"sha256":hash(&b)}))?;
       if caso=="escritor_desconectado" && i==0{return Err("ESCRITOR_DESCONECTADO_INYECTADO".into())}
      },
      Trabajo::Entrada(n,b)=>{if n!=ultimo+1{return Err("COLA_SECUENCIA".into())}ultimo=n;let mut f=OpenOptions::new().write(true).create_new(true).open(dir.join("entrada.txt"))?;f.write_all(&b)?;f.sync_all()?;
       d.evento("trabajo",json!({"ordinal":n,"clase":"entrada","archivo":"entrada.txt","bytes":b.len(),"sha256":hash(&b)}))?;},
      Trabajo::Evento(n,s,v)=>{if n!=ultimo+1{return Err("COLA_SECUENCIA".into())}ultimo=n;
       ...
       d.evento(&s,v.clone())?;
       d.evento("trabajo",json!({"ordinal":n,"clase":"evento","tipo":s,"datos":v}))?;},
  ...
  Ok(Self{tx,rx,fallo,error:None,fase:"recepcion",corte:0,tardios_bytes:0,snapshot:None})
 }
 ...
 pub fn consultar(&mut self){
  match self.rx.try_recv(){Ok(Ok(s))=>{self.snapshot=Some(s);self.fase="sellada";},Ok(Err(e))=>{self.error=Some(e);self.fallo.store(true,Ordering::SeqCst);self.fase="fallida";},Err(_)=>{}}
 }
}
