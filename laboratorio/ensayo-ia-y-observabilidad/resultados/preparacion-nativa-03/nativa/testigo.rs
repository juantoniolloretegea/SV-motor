...
fn main()->Result<(),Error>{
 ...
 if a[2]=="hijo_tardio"{std::thread::sleep(Duration::from_secs(4));writeln!(std::io::stdout(),"TARDIO_TRAS_PLAZO")?;return Ok(())}
 let rt=tokio::runtime::Runtime::new()?;
 let mut term=rt.block_on(async{tokio::signal::unix::signal(tokio::signal::unix::SignalKind::terminate())})?;
 iniciar_sink(a[1].clone())?;marca("inicio")?;
 if a[2]=="bloqueo_antes"||a[2]=="bloqueo_entre"{
  rt.block_on(term.recv());
  // Canal de observación independiente del journal bloqueado; sólo testigo.
  let socket=std::os::unix::net::UnixDatagram::unbound()?;socket.set_nonblocking(true)?;
  let _=socket.send_to(b"TERM_RECIBIDO","term-observador.sock");
  loop{std::thread::sleep(Duration::from_secs(1));}
 }

 if a[2]=="escritor_desconectado"{
  // Permanece vivo hasta TERM/KILL: la desconexión del escritor precede a waitpid.
  rt.block_on(term.recv());
  let socket=std::os::unix::net::UnixDatagram::unbound()?;socket.set_nonblocking(true)?;
  let _=socket.send_to(b"TERM_RECIBIDO","term-observador.sock");
  loop{std::thread::sleep(Duration::from_secs(1));}
 ...
 if a[2]=="duplicado"||a[2]=="orden"{
  let f=Frame{contrato:"EIO-NAT/2".into(),id:a[1].clone(),seq:if a[2]=="duplicado"{1}else{3},mono_ns:0,civil_unix_ms:civil(),tipo:"marca".into(),datos:json!({"nombre":"pesos.leidos"})};
 ...
 if a[2]=="tardio"{rt.block_on(term.recv());}
 ...
 if a[2]=="no_cero"{std::process::exit(77)}
 if a[2]=="eof_ausente"{
  std::process::Command::new(std::env::current_exe()?).args([&a[1],"hijo_tardio"]).stdin(std::process::Stdio::null()).stdout(std::process::Stdio::inherit()).stderr(std::process::Stdio::inherit()).spawn()?;
 }
 if a[2]=="tardio"{loop{std::thread::sleep(Duration::from_secs(1));}}
 Ok(())
}
