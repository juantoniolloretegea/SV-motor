...
fn main()->Result<(),Error>{
 ...
 let mut diagnosticos:Vec<Value>=Vec::new();let mut senales:Vec<Value>=Vec::new();
 let mut salida=Value::Null;let mut registro_waitpid_fallido=false;
 let mut parada_no_confirmada:Option<Instant>=None;
 // Diagnóstico volátil acotado; no sustituye al journal ni acredita persistencia.
 let anotar=|v:&mut Vec<Value>,fase:&str,e:String|{if v.len()<16{v.push(json!({"fase":fase,"error":e}));}};
 ...
 if let Err(e)=c.evento("inicio_supervision",json!({"id":id,"rss_umbral":RSS_LIMIT,"sondeo_ms":1000,"tick_ms":50,"modo":config.modo,"caso":config.caso})){fallo=true;anotar(&mut diagnosticos,"inicio_supervision",e.to_string());}
 loop{
  let mut motivo=None;
  ...
  if admision.fallo_custodia{fallo=true;cierre.revocada=true;admision.revocada=true;motivo=Some("CUSTODIA");}
  // Invalidación técnica por custodia, incluso después de recoger al hijo.
  // No implica que se haya enviado una señal a un proceso ya terminado.
  if let Some(e)=&escalada{for v in e.eventos.try_iter(){if senales.len()<8{senales.push(v.clone());}if let Err(err)=c.evento("senal",v){fallo=true;anotar(&mut diagnosticos,"registro_senal",err.to_string());}}}
  if sellando.is_some() && (c.snapshot.is_some()||admision.fallo_custodia){cerrado=true;estado=admision.estado();if admision.fallo_custodia{c.fase="fallida";}}
  if sellando.is_some_and(|t|t.elapsed()>Duration::from_secs(5))&&!cerrado{fallo=true;c.fallo.store(true,Ordering::SeqCst);c.error=Some("SELLADO_PLAZO".into());c.fase="fallida";admision.fallo_custodia=true;cierre.revocada=true;admision.revocada=true;cerrado=true;estado=admision.estado();}

  if shutdown.load(Ordering::SeqCst)||inicio.elapsed()>=Duration::from_secs(40*60){motivo=Some("CIERRE_SESION")}
  match server.child.try_wait(){
   Ok(Some(_))=>{motivo=Some("SERVIDOR_TERMINADO");},
   Ok(None)=>{},
   Err(e)=>{fallo=true;anotar(&mut diagnosticos,"observar_servidor",e.to_string());motivo=Some("SERVIDOR_OBSERVACION");shutdown.store(true,Ordering::SeqCst);}
  }
  ...
  if sample.elapsed()>=Duration::from_secs(1)&&!cerrado{
   let t=Instant::now();match medir(server.child.id(),task.as_ref().filter(|_|!reaped).map(|g|g.child.id()),page,guardian,&cg){
    Ok(m)=>{match m["total_rss"].as_u64(){Some(n) if n>RSS_LIMIT=>motivo=Some("RSS"),Some(_)=>{},None=>{fallo=true;motivo=Some("MEMORIA_DESCONOCIDA");}}ultimo=m;
  ...
  if let Ok((mut s,_))=listener.accept(){
   if let Err(e)=s.set_read_timeout(Some(Duration::from_millis(100))).and_then(|_|s.set_write_timeout(Some(Duration::from_millis(100)))){
    fallo=true;fault.store(true,Ordering::SeqCst);anotar(&mut diagnosticos,"socket_plazo",e.to_string());
    // No se usa un socket sin cotas; se continúa con parada/recogida.
    drop(s);
   }else{
   let reply=match recv::<Orden>(&mut s,MAX_INPUT){
    Ok(Orden::Estado)=>{
     ...
     json!({"id":id,"pid_supervisor":std::process::id(),"pid_inferidor":task.as_ref().map(|g|g.child.id()),"estado":if parada_no_confirmada.is_some(){"parada_no_confirmada"}else if cerrado{admision.estado()}else{estado},"admisible":admitido,"condiciones":admision,
     "estimulo":{"modo":config.modo,"caso":config.caso,"contrato":config.contrato},
     "reaped":reaped,"salida_proceso":salida,"parada_solicitada":escalada.is_some(),"senales":senales,
     "registro_waitpid_fallido":registro_waitpid_fallido,"diagnosticos_volatiles":diagnosticos,
     "error_custodia":c.error,"fase_custodia":c.fase,"sello":c.snapshot.as_ref().filter(|_|!c.fallo.load(Ordering::SeqCst)).map(|s|&s.sello),"evidencia_parcial":c.snapshot.as_ref().map(|_|!admitido),
  ...
  if let Some(g)=&mut task{
   if !reaped&&g.escalando.as_ref().is_none_or(|x|x.load(Ordering::SeqCst)){
    match g.child.try_wait(){
     Ok(Some(s))=>{
      reaped=true;exit_ok=s.success();salida=json!({"codigo":s.code(),"senal":s.signal(),"texto":s.to_string()});
      c.fase="drenaje";muerte.get_or_insert(Instant::now());
      if let Err(e)=c.evento("waitpid",json!({"status":s.to_string(),"pid":g.child.id(),"reaped":true})){
       registro_waitpid_fallido=true;fallo=true;c.fallo.store(true,Ordering::SeqCst);
       cierre.revocada=true;anotar(&mut diagnosticos,"registro_waitpid",e.to_string());
      }
     },
     Ok(None)=>{},
     Err(e)=>{
      fallo=true;exit_ok=false;cierre.revocada=true;
      anotar(&mut diagnosticos,"observar_hijo",e.to_string());
      // No se afirma reap. Se inicia escalada una sola vez y se conserva su plazo.
      if muerte.is_none(){let e=parada::iniciar(g.child.id());g.escalando=Some(e.terminada.clone());escalada=Some(e);muerte=Some(Instant::now());}
     }
    }
   }
   ...
   if !reaped&&muerte.is_some_and(|t|t.elapsed()>Duration::from_secs(5)){
    fallo=true;estado="parada_no_confirmada";parada_no_confirmada.get_or_insert(Instant::now());
    // Ventana diagnóstica acotada. Después actúan Drop y la guarda exterior.
    // La indisponibilidad NO es un negativo conforme del banco.
    if parada_no_confirmada.is_some_and(|t|t.elapsed()>=Duration::from_secs(2)){break}
   }
  }
