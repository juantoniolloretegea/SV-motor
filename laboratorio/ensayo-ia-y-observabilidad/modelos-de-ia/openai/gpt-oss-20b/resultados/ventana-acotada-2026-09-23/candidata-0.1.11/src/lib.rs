...
fn run(config: Config, root: PathBuf, evidence: PathBuf, journal: Journal) -> Result<Value> {
    ...
    let outcome = std::panic::catch_unwind(std::panic::AssertUnwindSafe(|| -> Result<()> {
        ...
        let topology = evidence.join("topologia.yaml");
        let topology_text = include_str!("../topologia.yaml");
        fs::write(&topology,topology_text).map_err(|e|e.to_string())?;
        journal.event("configuracion_carga",json!({"topology":topology_text,"topology_sha256":format!("{:x}",Sha256::digest(topology_text.as_bytes())),"isq":"q8_0","experts":"MXFP4 original conservado mediante selección explícita; sin recuantización de expertos","remaining_layers":"Q8_0 solicitado; identidad efectiva sujeta al registro del motor","single_isq_worker":true}))?;
        command.args(["--token-source","none","serve","-m","modelo","--cpu","--dtype","bf16","--isq","q8_0","--max-seq-len","1024","--max-seqs","1","--prefix-cache-n","0","--paged-attn","off","--host","127.0.0.1","--port",&port,"--no-ui","--topology"]).arg(&topology).env("MISTRALRS_ISQ_SINGLETHREAD","1");
        ...
        let data = models["data"].as_array().filter(|v|v.len()==1).ok_or("lista_modelos_no_univoca")?;
        let model = data[0]["id"].as_str().filter(|v|!v.is_empty()).ok_or("identidad_http_modelo_ausente")?;
        journal.event("servicio_comprobado",json!({"pid":pid,"start_ticks":start_ticks,"model":model,"check":"socket_atribuido_y_HTTP_models"}))?;
        let request = json!({"model":model,"messages":[{"role":"system","content":"Responda en español formal y preciso, en una sola frase. No use herramientas."},{"role":"user","content":"Caso sintético: un inventario tiene tres elementos y se añaden dos. ¿Cuántos elementos hay en total?"}],"max_tokens":96,"temperature":0,"reasoning_effort":"low","stream":false});
