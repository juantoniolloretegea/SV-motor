...
const repo=process.argv[2],base='laboratorio/ensayo-ia-y-observabilidad/modelos-de-ia/openai/gpt-oss-20b/resultados/onecloud-2026-09-24/optimizacion',dir=path.join(repo,base);
const leer=p=>JSON.parse(fs.readFileSync(path.join(dir,'evidencias',p),'utf8'));
const r=leer('REFERENCIA_RESUMEN.json'),p=leer('PARALELO_RESUMEN.json'),a=leer('AUDITORIA_INSTRUMENTAL.json');
assert(a.controles_aprobados&&a.umbral_rendimiento_cumplido);
const textos=['Hay cinco elementos en total.','Hay cinco elementos en total.','Hay cinco elementos en total.','3','AX-17','2, 5, 9','No, sin conocer la cantidad inicial no se puede determinar el total exacto.'];
assert.deepEqual(p.casos.map(c=>c.respuesta.choices[0].text),textos);
assert(r.casos.every(c=>c.respuesta.choices[0].text===textos[0]));
assert(fs.readFileSync(path.join(dir,'evidencias/CARGO_CHECK.log'),'utf8').includes('Finished `release`'));
const ahora=new Date().toISOString(),fecha=v=>new Date(v).toISOString(),n=v=>String(v).replace('.',','),s=v=>n((v/1000).toFixed(3));
const semantica={fecha_utc:ahora,metodo:'Cotejo explícito por la unidad de ejecución contra los oráculos publicados antes del banco. La comprobación de textos de este generador solo asegura transcripción; no constituye un evaluador semántico general.',casos:p.casos.map((c,i)=>({id:c.id,texto:c.respuesta.choices[0].text,correcto:true,justificacion:i<3?'Resultado aritmético cinco, sin contradicción.':['Siete menos cuatro es tres.','El código se conserva íntegro.','Orden ascendente completo, sin añadidos numéricos.','Reconoce la insuficiencia del dato inicial y no inventa un total.'][i-3]}))};
fs.writeFileSync(path.join(dir,'VALIDACION_SEMANTICA.json'),JSON.stringify(semantica,null,2)+'\n');
const tabla=p.casos.map((c,i)=>`| ${c.id} | ${i<3?s(r.casos[i].latencia_ms):'—'} | ${s(c.latencia_ms)} | ${c.respuesta.choices[0].text} |`).join('\n');
const informe=`# Optimización CPU MXFP4 y ejecución residente: resultado experimental

Registro documental: ${ahora}. Unidad de ejecución experimental. Seguimiento S39, revisión 18; Acta004 §18; RETP-2026-271; PTA-2026-013 y PTA-SVM-004. Verificación acotada de una investigación lateral; sin modificación del núcleo del lenguaje.

## Dictamen

Se cumple el criterio fijado antes de ejecutar: cinco casos correctos, incluidas tres repeticiones correctas del caso aritmético; cierre sin errores; mediana candidata no superior al 50 % de la referencia. La mediana pasa de **94,002 s a 18,168 s**: factor **${n(a.factor_aceleracion.toFixed(3))}**, reducción **${n(a.reduccion_porcentual.toFixed(3))} %**. La aceptación es local a este banco, configuración y anfitrión.
...
La [auditoría instrumental Rust](evidencias/AUDITORIA_INSTRUMENTAL.json) verifica ${a.controles.length} condiciones sobre los originales: cargas, procesos, solicitudes, respuestas, identidad de ejecutables, igualdad de peticiones comparables, HTTP 200, parámetros, orden temporal, ausencia de truncamiento, muestreo y cierre. Todas aprobadas. Esta auditoría no añade nuevos ensayos del modelo.

Cuota exterior por sesión: 32 GiB, swap 0, 256 tareas, red privada y escucha local. RLIMIT_AS interior: 32 GiB por proceso; no equivale a RSS agregada. Pico RSS muestreado del hijo: referencia ${r.finalizacion.data.peak_rss_bytes} B; candidata ${p.finalizacion.data.peak_rss_bytes} B. Muestras: ${r.finalizacion.data.rss_samples} y ${p.finalizacion.data.rss_samples}, sin muestras ausentes. No se equipara RSS con memory.peak del cgroup: el ámbito y la imputación de páginas compartidas y caché difieren.
...
`;
fs.writeFileSync(path.join(dir,'RESULTADO.md'),informe);
fs.writeFileSync(path.join(dir,'ESTADO.json'),JSON.stringify({estado:'finalizado_con_aceptacion_acotada',fecha_registro_utc:ahora,limite_rust_utc:'2026-09-24T12:58:00Z',limite_total_utc:'2026-09-24T13:58:00Z',fin_banco_utc:fecha(p.finalizacion.utc_ms),criterio_rendimiento:true,criterio_semantico:true,errores_de_cierre:0,alternativa_llamacpp:'No activada; condición de paso no satisfecha.',inferencia_activa:false,ramas_creadas:0,recursos_adquiridos:0,siguiente_accion:'Conservar revisión candidata; cualquier ampliación de tareas o contexto requiere protocolo y plazo nuevos.',trazabilidad:'S39 revisión 18; Acta004 §18; RETP-2026-271; PTA-2026-013; PTA-SVM-004'},null,2)+'\n');
const csv=v=>v.map(x=>'"'+String(x).replaceAll('"','""')+'"').join(',');
const pc='docs/calidad/REGISTRO_PARTES_DE_TRABAJO_POR_AGENTE.csv',pm=pc.replace('.csv','.md');
const linea=['PTA-SVM-004','2026-09-24',ahora.slice(11,19)+' UTC','Unidad de ejecución experimental','VERIFICACION_ACOTADA','Optimización CPU MXFP4 y ejecución residente','Protocolo 7a3a353; fuentes 388933b; referencia d097203','Latencia elevada en la candidata corregida','Construcción apareada; tres pruebas numéricas; nueve guardas; diez peticiones en dos sesiones; auditoría; cierre','Fuentes y originales de optimización; informe; manifiesto; registro de partes','Aceptación acotada: cinco casos correctos y mediana 94,002 a 18,168 s','Sin adquisición ni ramas adicionales; llama.cpp no activado; S39 rev18 / Acta004 §18 / RETP271 / PTA013'];
assert(!fs.readFileSync(path.join(repo,pc),'utf8').includes('PTA-SVM-004'));
fs.appendFileSync(path.join(repo,pc),csv(linea)+'\n');
fs.appendFileSync(path.join(repo,pm),`\n## PTA-SVM-004 · Optimización y ejecución residente\n\nRegistro ${ahora}. Unidad de ejecución experimental; VERIFICACION_ACOTADA.\n\nSe compila un único motor con referencia secuencial y variante paralela; se conservan tres pruebas numéricas, nueve guardas, Cargo check, diez peticiones en dos sesiones, auditoría instrumental y cierre. Cinco casos aceptados; mediana 94,002 a 18,168 s. Cumplido el criterio de primera fase, no se activa llama.cpp. No hay inferencia activa ni compra de recursos.\n\n[Informe y límites](../../${base}/RESULTADO.md). S39 revisión 18; Acta004 §18; RETP-2026-271; PTA-2026-013. Sin certificación general ni modificación doctrinal.\n`);
console.log('Informe, estado, validación semántica y parte de trabajo preparados.');
