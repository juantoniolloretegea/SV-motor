...
fn run(config: Config, root: PathBuf, evidence: PathBuf, journal: Journal) -> Result<Value> {
    ...
    let outcome = std::panic::catch_unwind(std::panic::AssertUnwindSafe(|| -> Result<()> {
        ...
        command.args(["--token-source","none","serve","-f","gguf/gpt-oss-20b-MXFP4.gguf","--tok-model-id","modelo","--cpu","--device-layers","0","--dtype","f32","--max-seq-len","1024","--max-seqs","1","--prefix-cache-n","0","--paged-attn","off","--host","127.0.0.1","--port",&port,"--no-ui"]);
        ...
        command.current_dir(&root).env("TIKTOKEN_ENCODINGS_BASE",root.join("harmony")).env("HF_HUB_OFFLINE","1").env("MISTRALRS_CPU_KV_F32","1").env_remove("MCP_CONFIG_PATH").stdin(Stdio::null()).stdout(log.try_clone().map_err(|e|e.to_string())?).stderr(log).process_group(0);
