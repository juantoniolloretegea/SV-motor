...
fn observe(pid:u32,start:u64,dir:&Path,period:u64,seconds:u64)->Result<()>{
 ...
 loop{
  ...
  if running{let data=tree(pid,start,std::process::id());let engine=engine_snapshot();let disk=nix::sys::statvfs::statvfs(dir).ok().map(|s|s.blocks_available().saturating_mul(s.fragment_size()));t.event(None,"observacion_linux",json!({"target_pid":pid,"target_start_ticks":start,"sample":samples,"tree":data,"resident_engine":engine,"observer":snapshot(std::process::id(),identity(std::process::id())?).ok(),"filesystem_available_bytes":disk,"clock_ticks_per_second":nix::unistd::sysconf(nix::unistd::SysconfVar::CLK_TCK).ok().flatten()}));samples+=1;
  }else{t.event(None,"observador_finalizado",json!({"target_pid":pid,"target_start_ticks":start,"reason":reason}));}
  checkpoint(&dir.join("estado.json"),&json!({"schema":"EIO-OBSERVADOR-1","pid":std::process::id(),"target_pid":pid,"target_start_ticks":start,"utc_ms":store::now(),"running":running&&t.healthy(),"reason":if t.healthy(){reason}else{"exportacion_no_integra"},"samples":samples,"period_seconds":period,"maximum_seconds":seconds,"export":t.status(),"scope":"Muestreo del servicio y descendientes; el motor residente se observa separadamente mediante su cgroup. No censo del anfitrión, tráfico completo ni auditoría de llamadas al sistema."}))?;
...
fn engine_snapshot()->Value{
 let path="/sys/fs/cgroup/system.slice/sv-conversacion-motor.service/cgroup.procs";
 match bounded(path,8192){Ok(s)=>{let mut rows=vec![];let mut failures=vec![];let pids:Vec<_>=s.split_whitespace().collect();for p in pids.iter().take(16){match p.parse::<u32>().map_err(|e|e.to_string()).and_then(|pid|identity(pid).and_then(|start|snapshot(pid,start)).map_err(|e|e.to_string())){Ok(v)=>rows.push(v),Err(e)=>failures.push(e)}}json!({"scope":"Procesos de la unidad residente; observación separada del árbol del servicio","processes":rows,"incomplete":failures,"truncated":pids.len()>16})},Err(e)=>json!({"available":false,"reason":e.to_string()})}
}
