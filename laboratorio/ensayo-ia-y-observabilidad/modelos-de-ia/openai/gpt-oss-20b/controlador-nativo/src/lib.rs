...
fn run(config: Config, root: PathBuf, evidence: PathBuf, journal: Journal) -> Result<Value> {
    ...
    let outcome = std::panic::catch_unwind(std::panic::AssertUnwindSafe(|| -> Result<()> {
        ...
        command.args(["--token-source","none","serve","-m","modelo","--cpu","--dtype","bf16","--max-seq-len","1024","--max-seqs","1","--prefix-cache-n","0","--paged-attn","off","--host","127.0.0.1","--port",&port,"--no-ui"]);
