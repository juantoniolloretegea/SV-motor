# Modelos de IA

**Edición:** 2 · 23 de septiembre de 2026.  
**Ámbito:** catálogo experimental del ensayo de inteligencia artificial y observabilidad.

El modelo, el motor numérico y la vía de ejecución son elementos distintos. Este catálogo identifica su relación y remite a las fuentes y evidencias existentes. Las instalaciones y los resultados conservan su ubicación; no se duplican pesos, dependencias ni expedientes.

| Ficha | Modelo | Vía B · nativa | Vía A · navegador/WebAssembly |
|---|---|---|---|
| [Qwen3-0.6B](qwen/qwen3-0.6b/README.md) | Qwen3-0.6B, Q4_K_M | Conversación 0.1.3, Beta 1; realización parcial verificada. | Antecedentes NAV-02; inferencia real completa no acreditada. |
| [gpt-oss-20b](openai/gpt-oss-20b/README.md) | gpt-oss-20b | Sin instalación en este ensayo. | Candidato sujeto a viabilidad; no se ha ejecutado. |

Las fichas identifican versiones concretas, estado, requisitos y límites. Un nombre de proveedor no acredita equivalencia entre modelos. La [composición distribuida](../README.md#versión-distribuida), el [contrato experimental](../contrato/README.md) y la evidencia de cada ejecución mantienen su función propia.

La incorporación de otro modelo exige una necesidad delimitada y recursos disponibles dentro de la cuota gratuita de la cuenta. El criterio es gasto monetario cero, tanto en cómputo como en almacenamiento. No genera por sí sola una nueva rama, un entorno o una campaña. El estado de instalación aquí documentado no indica que exista un servicio en ejecución permanente.

## Organización por marca y modelo

Cada modelo tiene su propia carpeta: `qwen/qwen3-0.6b/` y `openai/gpt-oss-20b/`. Los índices [Qwen](qwen/README.md) y [OpenAI](openai/README.md) conservan las rutas anteriores y permiten incorporar otros modelos sin ocupar toda la carpeta de la marca.

La reorganización del 23/09/2026 afecta únicamente a estas fichas y sus enlaces. Los estados experimentales conservan su corte documental; no se trasladan instalaciones, pesos, ejecutables ni evidencias. Esta organización no autoriza un encargo ni una ejecución.
