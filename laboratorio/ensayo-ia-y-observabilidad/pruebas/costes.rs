...
fn main()->Result<(),Box<dyn std::error::Error>>{
 ...
 for (i,modo) in ["on","on","off","off","on","on","off"].iter().enumerate(){
  ...
  let s=r["salida"]["segundos"].as_f64().ok_or("tiempo ausente")?;
  ...
  fila.push(serde_json::json!({"numero":i+1,"modo":modo,"calentamiento":i==0,
   "segundos":s,"pico_rss_kib":pico,"tokens":r["salida"]["tokens"],"contrato":r["contrato"]}));
 ...
 "tokens_equivalentes":equivalentes,"limite":"Tiempo del adaptador: incluye carga y tokenización; no toda la vida del proceso. RSS muestreado; null no acredita medida."}));
